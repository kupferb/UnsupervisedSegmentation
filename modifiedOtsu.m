function [level, m1, em] = modifiedOtsu(I)
...
%
%   Leonardo Rundo modification:
%   Backgroung level occurences removal for masked images
%
...
%   $Revision: 1.9.4.11 $  $Date: 2011/08/09 17:50:01 $
%   Modified by Leonardo Rundo
...
if ~isempty(I)
  ...
  % Background component elimination
  counts(1) = 0;
  ...
  if isfinite_maxval
    ...
    % Mean calculation
    H1_pixel_number = sum(counts(1:idx));
    % Background Integrator
    weighted_H1_sum = 0;
    for i = 1:idx
        weighted_H1_sum = weighted_H1_sum + counts(i) * (i-1);
    end
     % Mean value of the region H1
    H1_mean = weighted_H1_sum / H1_pixel_number;
    
    % Normalize the threshold to the range [0, 1].
    level = (idx - 1) / (num_bins - 1);
    m1 = (round(H1_mean) - 1) / (num_bins - 1);
  else
    level = 0.0;
  end
else
